...
function resetWebsiteState() {
    ...
    if (typeof S !== 'undefined' && S.UI && window.settings) {
        S.UI.reset(true);
        const sequence = `|${window.settings.sequence}|#gift|`;
...
function applyLoadedSettings() {
    ...
    S.UI.reset(true);
    const sequence = `|${settings.sequence}|#gift|`;
    S.UI.simulate(sequence);
...
function populateModal() {
    ...
    // Thêm event listener untuk tombol warna
    ...
    if (enableBookSelect) {
        enableBookSelect.addEventListener('change', function () {
            ...
            if (this.value === 'true') {
                ...
                if (enableHeartEl) enableHeartEl.disabled = false;
            } else {
                if (bookSettingsSection) bookSettingsSection.style.display = 'none';
                if (enableHeartEl) { enableHeartEl.value = 'false'; enableHeartEl.disabled = true; }
            }

            if (typeof updatePricingFromModal === 'function') updatePricingFromModal();
        });
    }
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            window.settings = {
                ...
                sequence: serverSettings.sequence || 'HAPPY|BIRTHDAY|DEWI|❤',
...
if (applySettingsButton) applySettingsButton.addEventListener('click', () => {
    ...
    const bgMusicEl = document.getElementById('backgroundMusic');
    if (bgMusicEl) settings.music = bgMusicEl.value;
    const countdownEl = document.getElementById('countdownTime');
    if (countdownEl) settings.countdown = parseInt(countdownEl.value) || 3;
    const matrixTextEl = document.getElementById('matrixText');
    if (matrixTextEl) settings.matrixText = matrixTextEl.value || 'HAPPYBIRTHDAY';
    const matrixColor1El = document.getElementById('matrixColor1');
    if (matrixColor1El) settings.matrixColor1 = matrixColor1El.value;
    const matrixColor2El = document.getElementById('matrixColor2');
    if (matrixColor2El) settings.matrixColor2 = matrixColor2El.value;
    const sequenceTextEl = document.getElementById('sequenceText');
    if (sequenceTextEl) settings.sequence = sequenceTextEl.value || 'HAPPY|BIRTHDAY|DEWI|❤';
    const sequenceColorEl = document.getElementById('sequenceColor');
    if (sequenceColorEl) settings.sequenceColor = sequenceColorEl.value;
    const giftEl = document.getElementById('giftImage');
    if (giftEl) settings.gift = giftEl.value;
    ...
    const enableBookEl = document.getElementById('enableBook');
    if (enableBookEl) settings.enableBook = enableBookEl.value === 'true';
    const enableHeartEl2 = document.getElementById('enableHeart');
    if (enableHeartEl2) settings.enableHeart = enableHeartEl2.value === 'true';
